...
import applogo from "../assets/HoYo1.png"
...
const Navbar = () => {
    ...
    const handleDeleteAccount = async () => {
        if (window.confirm("Are you sure you want to delete your account? This action cannot be undone.")) {
            try {
                ...
                // Log out from Clerk after deleting from your backend
                await signOut();
            } catch (error) {
                console.error(error);
    ...
    return (
        <nav className={`mb-10 fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
            <Link to="/">
                <img src={applogo} alt="logo" className={`h-9 w-60 h-40 ${isScrolled && "invert opacity-80"}`} />
            </Link>
            ...
            <div className="hidden md:flex items-center gap-4">
                {user ? (
                    ...
                    <button onClick={openSignIn} className="bg-black text-white px-10 py-2.5 rounded-full ml-4 transition-all duration-500 cursor-pointer">
                        Login
                    </button>
                )}
            </div>
